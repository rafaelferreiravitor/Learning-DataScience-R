...
library(rpart)
regressor = rpart(formula = custo ~ idade, data = base, control = rpart.control(minsplit = 1)) # o control é para forçar a divisão a ocorrer pois como a base é pequena ele classificou tudo em um so nó
summary(regressor)
