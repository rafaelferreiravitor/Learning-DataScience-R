...
table(base$sex) # apresenta todos os valores possíveis do atributo e a quantidade de cada um deles
unique(base$sex) # apresenta todos os valores possíveis do atributo

# Para modificar os valores categoricos para valores numéricos pode-se utilizar as seguintes funções:
...
base[is.na(base$sex)] #recomenda-s verificar se apos a migração nenhum valor ficou como NA


# Restante da migração dos valores categoricos para numéricos
...
# Deve-se escalonar todos atributos que eram nativamente numéricos (nao e possivel escalonar atributos que foram migrados)
base[, 1] = scale(base[, 1])
base[, 3] = scale(base[, 3])
base[, 5] = scale(base[, 5])
